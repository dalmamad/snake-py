...
class Option:
    ...
    def draw_txt(cls):
        keys = ['w,a,s,d', 'arrows']
        lines = ['in 2 player the lose is a snake',
                 'that collides with itself or another snake']
        for index in range(2):
            txt = font.render('snake' + str(index + 1) +
                              ' keys : ' + keys[index], True, cls.txtcolor)
            txt_width = txt.get_width()
            txt_height = txt.get_height()
            screen.blit(txt, ((WIN_WIDTH-txt_width)/2, 0.2 *
                        WIN_HEIGHT + index*txt_height*1.3))
        for index in range(len(lines)):
            txt = font.render(lines[index], True, cls.txtcolor)
            txt_width = txt.get_width()
            txt_height = txt.get_height()
            screen.blit(txt, ((WIN_WIDTH-txt_width)/2, 0.2 *
                        WIN_HEIGHT + 2*txt_height*1.5 + index*txt_height*1.3))
...
Option.options.append(Option(0, '1-player', OPTION_BGCOLOR, OPTION_SELECT_COLOR,
                      OPTION_TXTCOLOR, 1, OPTION_BGLEN, OPTION_BGPOS, OPTION_BGGAP))
Option.options.append(Option(1, '2-player', OPTION_BGCOLOR, OPTION_SELECT_COLOR,
                      OPTION_TXTCOLOR, 2, OPTION_BGLEN, OPTION_BGPOS, OPTION_BGGAP))
Option.options.append(Option(2, 'exit', OPTION_BGCOLOR, OPTION_SELECT_COLOR,
                      OPTION_TXTCOLOR, 0, OPTION_BGLEN, OPTION_BGPOS, OPTION_BGGAP))
