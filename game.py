...
class Snake():
    ...
    def add_init_parts(self):
        ...
        for index in range(1, self.len ):
            self.parts.append((self.parts[index -1][0] + dx, self.parts[index -1][1] + dy))
    def turn(self, key):
            ...
            if index > 2:
                if head[0] == part[0] and head[1] == part[1]:
                    ...
                    for index in range(self.__class__.snakes_count):
                        game_info['score_'+str(index+1)] = self.__class__.snakes[index].score
                        game_info['snake'+str(index+1)+'_color'] = self.__class__.snakes[index].color
                        if index != self.index:
    ...
    def smooth_draw(self):
        ...
        for index, part in enumerate(self.parts):
            if index == 0:
                if self.dir == UP:
                    pygame.draw.rect(screen, self.color, pygame.Rect(part[0] * GRID_SIZE , (part[1]+1) * GRID_SIZE  - layer, GRID_SIZE,  layer*1.5))
                if self.dir == DOWN:
                    pygame.draw.rect(screen, self.color, pygame.Rect(part[0] * GRID_SIZE , part[1] * GRID_SIZE , GRID_SIZE, layer))
                if self.dir == LEFT:
                    pygame.draw.rect(screen, self.color, pygame.Rect((part[0]+1) * GRID_SIZE -layer , part[1] * GRID_SIZE , layer*1.5, GRID_SIZE ))
                if self.dir == RIGHT:
                    pygame.draw.rect(screen, self.color, pygame.Rect(part[0] * GRID_SIZE, part[1] * GRID_SIZE , layer, GRID_SIZE ))
            elif index == last_index:
                if self.eat == False:
                    if self.tail_dir == UP:
                        pygame.draw.rect(screen, self.color, pygame.Rect(part[0] * GRID_SIZE , part[1] * GRID_SIZE  , GRID_SIZE, GRID_SIZE - layer))
                    if self.tail_dir == DOWN:
                        pygame.draw.rect(screen, self.color, pygame.Rect(part[0] * GRID_SIZE , part[1] * GRID_SIZE + layer , GRID_SIZE, GRID_SIZE ))
                    if self.tail_dir == LEFT:
                        pygame.draw.rect(screen, self.color, pygame.Rect(part[0] * GRID_SIZE , part[1] * GRID_SIZE , GRID_SIZE - layer, GRID_SIZE ))
                    if self.tail_dir == RIGHT:
                        pygame.draw.rect(screen, self.color, pygame.Rect(part[0] * GRID_SIZE + layer , part[1] * GRID_SIZE , GRID_SIZE , GRID_SIZE ))
            elif index != 0:
                pygame.draw.rect(screen, self.color, pygame.Rect(part[0] * GRID_SIZE , part[1] * GRID_SIZE , GRID_SIZE, GRID_SIZE))
...
class Food():
    ...
    def check_eated(self,snakes):
        if self.eated:
            ...
            self.create(snakes)
    def draw(self):
        pygame.draw.rect(screen, self.color, pygame.Rect(self.pos[0] * GRID_SIZE , self.pos[1] * GRID_SIZE , GRID_SIZE, GRID_SIZE)) 
